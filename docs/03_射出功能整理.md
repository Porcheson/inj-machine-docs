...
flowchart TD
    ...
    Idle -->|射出模式| Injecting[射出中 10段]
    Injecting -->|射出10段完成| Injected[射出完成]
    Injected -->|转保压条件| Holding[保压中 8段]
    Holding -->|保压8段完成| Held[保压完成]
    Held -->|射出完成信号| End[射出流程结束]

    Injecting -->|错误| Error[错误状态]
    Holding -->|错误| Error

    Error -->|复位| Idle

    class Start,Init,Idle,Injecting,Injected,Holding,Held state
    class Error error
```
...
sequenceDiagram
    ...
    射出控制->>执行机构: 射出1段动作
    射出控制->>射出控制: 射出2段 (eState_Injecting)
    射出控制->>执行机构: 射出2段动作
    Note over 射出控制: ... 射出3-9段 ...
    射出控制->>射出控制: 射出10段
    射出控制->>执行机构: 射出10段动作
    执行机构-->>射出控制: 射出完成信号
    射出控制->>射出控制: 保压1-8段 (eState_Holding)
    射出控制->>执行机构: 保压1段动作
    执行机构-->>射出控制: 保压1段时间到
    射出控制->>执行机构: 保压2段动作
    Note over 射出控制: ... 保压2-7段 ...
    射出控制->>执行机构: 保压8段动作
    执行机构-->>射出控制: 保压完成信号
...
flowchart TD
    Start[初始状态] -->|启动信号| Init[初始化]
    Init -->|初始化完成| Idle[空闲状态]
    Idle -->|射退模式| SuckBacking[射退中]
    SuckBacking -->|到位检测| SuckBacked[射退完成]
    SuckBacking -->|超时| Error[错误状态]
    Error -->|复位| Idle
    SuckBacked -->|复位| Idle

    class Start,Init,Idle,SuckBacking,SuckBacked state
    class Error error
```
